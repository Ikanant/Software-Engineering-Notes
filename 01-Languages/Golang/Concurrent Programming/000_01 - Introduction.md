...
-   Communicating Sequential Process (The Actor Model)
    ...
    -   Allows massive concurrency with amazing relaiability
...
A **Process** has one or more execution **Threads**. A Thread consist of Sequences of Programming instructions that execute in order... These are what is actually running an application in the context of a process. [All threads share the same memory space, which is managed by the process.]{.underline}
